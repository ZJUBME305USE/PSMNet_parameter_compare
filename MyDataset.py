...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask_right = (right_z > 5)& (right_z < 5 * med_right)
        disp_left = np.zeros_like(mask_left).astype(np.float32)
        disp_right = np.zeros_like(mask_right).astype(np.float32)
        ...
        disp_right[mask_right] = bb * ff / right_z[mask_right]
        sample['disp'] = disp_left[..., np.newaxis]
        sample['mask'] = mask_left[..., np.newaxis]
        ...
        if self.phase == 'train':
            if self.transform is not None:
                ...
                # ds_left = self.transform(ds_left)
