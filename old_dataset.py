...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # rec_left_gt[np.where(np.isnan(rec_left_gt))] = 0
        # rec_right_gt[np.where(np.isnan(rec_right_gt))] = 0
        # rec_left_gt[np.where(np.isinf(rec_left_gt))] = 0
        # rec_right_gt[np.where(np.isinf(rec_right_gt))] = 0
        # if np.sum(np.isinf(rec_left_gt).astype(np.float32)) > 0 or np.sum(np.isinf(rec_right_gt).astype(np.float32)) > 0 or np.sum(np.isnan(rec_left_gt).astype(np.float32)) > 0 or np.sum(np.isnan(rec_right_gt).astype(np.float32)) > 0:
        #     print("has inf or nan element")
        ...
        return [img_to_tensor(ds_left), img_to_tensor(ds_right), img_to_tensor(rec_left_gt), img_to_tensor(rec_right_gt),\
               img_to_tensor(mask_left), img_to_tensor(mask_right), torch.from_numpy(reprojection_matrix_left), torch.from_numpy(reprojection_matrix_right), torch.from_numpy(bb), torch.from_numpy(ff)]
