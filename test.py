...
parser.add_argument('--datapath', default='/home/greatbme/data_plus/Stereo Dataset',
                    help='select model')
parser.add_argument('--loadmodel', default='/home/greatbme/public/HX/PSMNet_experiment/depth_estimation_training_run_6_16_15_valid_2/checkpoint_6.tar',
                    help='loading model')
...
def Test(ds_left, ds_right, rec_left_gt, rec_right_gt, mask_left, mask_right,  reproj_left, reproj_right, bb, ff):
    model.eval()

    if args.cuda:
        ds_left, ds_right, rec_left_gt, rec_right_gt, mask_left, mask_right, reproj_left, reproj_right, bb, ff = ds_left.cuda(), ds_right.cuda(), rec_left_gt.cuda(), rec_right_gt.cuda(), mask_left.cuda(), mask_right.cuda(),  reproj_left.cuda(), reproj_right.cuda(), bb.cuda(), ff.cuda()
...
if __name__ == '__main__':
    ...
    all_count = 0
    val_step = 0
    for batch_idx, (ds_left, ds_right, rec_left_gt, rec_right_gt, mask_left, mask_right, reproj_left, reproj_right, bb,ff) in enumerate(ValidationImgLoader):
        mae_xyz, mae_z, bad10, bad2, bad1, bad05, count = Test(ds_left, ds_right, rec_left_gt, rec_right_gt,
                                                                  mask_left, mask_right, reproj_left, reproj_right, bb,ff)
        ...
        if mae_xyz < 0:
            ...
            val_step += 1
            writer.add_scalar('Validation/epoch{}_mae_xyz'.format(6), mae_xyz, val_step)
            writer.add_scalar('Validation/epoch{}_mae_z'.format(6), mae_z, val_step)
            writer.add_scalar('Validation/epoch{}_bad10'.format(6), bad10, val_step)
            writer.add_scalar('Validation/epoch{}_bad2'.format(6), bad2, val_step)
            writer.add_scalar('Validation/epoch{}_bad1'.format(6), bad1, val_step)
            writer.add_scalar('Validation/epoch{}_bad05'.format(6), bad05, val_step)
    ...
    writer.add_scalar('epoch{}_mean_bad10'.format(6), mean_bad10, 1)
    writer.add_scalar('epoch{}_mean_bad2'.format(6), mean_bad2, 1)
    writer.add_scalar('epoch{}_mean_bad1'.format(6), mean_bad1, 1)
    writer.add_scalar('epoch{}_mean_bad05'.format(6), mean_bad05, 1)
    writer.add_scalar('epoch{}_mean_mean_mae_xyz'.format(6), mean_mae_xyz, 1)
    writer.add_scalar('epoch{}_mean_mean_mae_z'.format(6), mean_mae_z, 1)
